...
function fmtBoard(id, b) {
  const watch = b.watchChannelIds?.length
    ? b.watchChannelIds.map((x) => `<#${x}>`).join(", ")
    : "(none)";
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sc) =>
      sc
        ...
        .addStringOption((o) => o.setName("emoji").setDescription("Emoji (⭐ or <:name:id>)").setRequired(false))
        .addIntegerOption((o) =>
    ...
    .addSubcommand((sc) =>
      sc
        ...
        .addIntegerOption((o) =>
          o.setName("threshold").setDescription("Threshold").setMinValue(1).setMaxValue(50).setRequired(false)
        )
    ...
    try {
      ...
      if (sub === "enable") {
        ...
        return replyEphemeral(
          interaction,
          `✅ Starboards globally ${enabled ? "enabled" : "disabled"}. (${Object.keys(next.boards).length} board(s))`
        );
      }
      ...
      if (sub === "watch-add") {
        ...
        setStarboardsConfig(interaction.guildId, { boards });
